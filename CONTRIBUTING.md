...
- **Weapon** → row in [`src/data/weapons.ts`](src/data/weapons.ts) → sprite-icon bake in [`src/art/sprites/icons/weapons.ts`](src/art/sprites/icons/weapons.ts) → i18n EN+SCS leaves (`weapon.<key>.name` + flavour) → if evolved: recipe in [`EVOLUTION_RECIPES`](src/core/BalanceConfig.ts) (`BURNS_EVOLUTION_THRESHOLD` is hand-pinned and must be bumped deliberately together with the achievement copy) → upgrade-pool card in [`src/data/upgrades.ts`](src/data/upgrades.ts) → banter `weapon_evolve` sub-tag if voiceful.
