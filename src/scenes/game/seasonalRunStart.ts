...
/**
 * Build a seasonal plan: an inert plan (all extras 0) with the event's toast
 * and only the stat fields the event actually sets spread over the top. A
 * branch names just its live field(s); every other extra defaults to 0, so
 * there are no hand-written `extra*: 0` lines to copy-paste wrong.
 */
function blessingPlan(
  seasonalEventKey: string,
  toastKey: string,
  color: string,
  extra: Partial<SeasonalRunStartPlan>,
): SeasonalRunStartPlan {
  return { ...inertPlan(seasonalEventKey), toast: toast(toastKey, color), ...extra };
}
...
export function buildSeasonalRunStartPlan(deps: SeasonalRunStartDeps): SeasonalRunStartPlan {
  ...
  if (firstFootingResult.gift) {
    return blessingPlan(seasonalEventKey, `ui.firstFooting.toast.${firstFootingResult.gift}`, '#f0d090', {
  ...
  if (beltaneResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.beltane.blessing_toast', '#f0a060', {
  ...
  if (samhainResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.samhain.blessing_toast', '#a060c0', {
  ...
  if (standrewsResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.standrews.blessing_toast', '#5a8acc', {
  ...
  if (burnsResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.burnsNight.blessing_toast', '#c89060', {
  ...
  if (imbolcResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.imbolc.blessing_toast', '#f5e7b8', {
  ...
  if (lammasResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.lammas.blessing_toast', '#d4a040', {
  ...
  if (brackenResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.brackenTurn.blessing_toast', '#b87038', {
  ...
  if (bannockburnResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.bannockburn.blessing_toast', '#a8c0d0', {
  ...
  if (gloriousTwelfthResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.gloriousTwelfth.blessing_toast', '#9c8838', {
  ...
  if (tartanDayResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.tartanDay.blessing_toast', '#b04050', {
  ...
  if (simmerDimResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.simmerDim.blessing_toast', '#9080c0', {
  ...
  if (upHellyAaResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.upHellyAa.blessing_toast', '#e07840', {
  ...
  if (cullodenResult.applied) {
    ...
    return blessingPlan(seasonalEventKey, 'ui.culloden.memorial_toast', '#708090', {});
  }

  const highlandGamesResult = applyHighlandGamesBlessing(seasonalEventKey, deps.runModifiers);
  if (highlandGamesResult.applied) {
    return blessingPlan(seasonalEventKey, 'ui.highlandGames.blessing_toast', '#d4a820', {
