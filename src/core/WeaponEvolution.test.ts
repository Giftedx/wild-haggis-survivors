...
describe('weapon evolution (chest-gated)', () => {
  ...
  it('covers all 18 evolution recipes (not accidentally truncated)', () => {
    ...
    // (hagstone_sling + rowan_amulet). If this count changes, double-check
    // the BURNS threshold sibling test in `BalanceConfig.evolution.test.ts`.
    expect(EVOLUTION_RECIPES.length).toBe(18);
  });
});
