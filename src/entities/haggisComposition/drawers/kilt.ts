...
function drawKilt(g: Phaser.GameObjects.Graphics, frame: KiltFrame): void {
  ...
  // ── Main field — rectangular wrap covering belly-to-leg span.
  // Slightly wider at the bottom (the pleats open outward). ──
  g.fillStyle(KILT_RED_DARK, 1);
  g.fillRect(cx - 15, cy - 8, 30, 14);
  g.fillStyle(KILT_RED, 1);
  g.fillRect(cx - 14, cy - 8, 28, 13);

  // ── Warp stripes — forest green vertical bars ──
  g.fillStyle(KILT_GREEN, 0.85);
  g.fillRect(cx - 11, cy - 8, 2, 13);
  g.fillRect(cx - 3, cy - 8, 2, 13);
  g.fillRect(cx + 5, cy - 8, 2, 13);
  g.fillRect(cx + 11, cy - 8, 1, 13);
  ...
  g.fillRect(cx - 9, cy - 8, 1, 13);
  g.fillRect(cx - 1, cy - 8, 1, 13);
  g.fillRect(cx + 7, cy - 8, 1, 13);
