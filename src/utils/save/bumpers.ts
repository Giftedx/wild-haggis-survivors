...
/**
 * Clootie Rag Wager (DESIGN_IDEAS §1) — bump the lifetime committed-wager
 * counter and return the **pre-bump** value. Caller uses pre-bump 0 to
 * gate the `bound_first` banter sub-pool (first wager ever = supplication
 * wonder; subsequent = ritual). Sister to `bumpBeithirCured`. v21
 * addition. Storage failures return `Number.MAX_SAFE_INTEGER` so the
 * first-wager gate (`=== 0`) never trips falsely on a corrupted save.
 */
export function bumpClootieWagerCommit(): number {
  try {
    const cur = loadSave();
    const before = cur.clootieWagersLifetime ?? 0;
    writeSave({ ...cur, clootieWagersLifetime: before + 1 });
    return before;
  } catch {
    return Number.MAX_SAFE_INTEGER;
  }
