...
const LOC_BUDGET: ReadonlyArray<readonly [string, number]> = [
  ...
  ['data/banter.ts', 2620], // bumped 2026-05-09 (2565→2620) for beithir_sting pool (DESIGN_IDEAS §1 Race the Beithir — context union member + pool entry, edge tone, priority 90, top-level keys carry `stung` lines + three sub-pool tags `cured_heal` / `cured_kill` / `expired`); current 2599 + 21 grace; pure data, parity-fenced, kept whole. Earlier bumps same day: 2535→2565 (lemmings_remember), 2490→2535 (taxman_grudge), 2455→2490 (clootie_wager), 2430→2455 (shinty_parry), 2400→2430 (stance_change), 2375→2400 (simmer_dim seasonal)
  ...
  ['entities/Player.ts', 1840], // bumped 2026-05-09 (1720→1840) for Race the Beithir (DESIGN_IDEAS §1 — beithirState field + maxHpAtSting lock + tickBeithir per-frame drain in update + applyBeithirStingFromFang / cureBeithirStingFromHeal / cureBeithirStingFromKill / fireBeithirExpired methods + isBeithirStung / beithirRemainingFraction HUD readouts); current 1825 + 15 grace; hot path. Earlier bumps same day: 1705→1720 (Clootie Rag Wager), 1620→1705 (Shinty Parry), 1556→1620 (Stance Toggle)
  ...
  ['systems/WeaponSystem.ts', 1415], // bumped 2026-05-09 (1385→1415) for Shinty Stick weapon (DESIGN_IDEAS §1+§5 — second bouncing-behavior projectile via shinty_ball texture; fireBouncing forks on weapon key, fireRapidBounce parameterised to share dispatch with Caman Storm evolution, applyProjectileVisual gains shinty_ball case); current 1405 + 10 grace; orchestrator, no obvious sub-system seams. Earlier bump same day (1360→1385) for Taxman Grudge Ledger emit
