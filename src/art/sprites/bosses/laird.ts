...
export function bakeBossLaird(scene: Phaser.Scene): void {
  ...
  g.fillRect(cx - 11, cy + 26, 8, 8);
  g.fillRect(cx + 3, cy + 26, 8, 8);
  g.fillStyle(0x1a4a1a, 1);
  g.fillRect(cx - 11, cy + 26, 8, 7);
  g.fillRect(cx + 3, cy + 26, 8, 7);
  g.fillStyle(0x2a5a22, 1);
  g.fillRect(cx - 10, cy + 27, 6, 5);
  g.fillRect(cx + 4, cy + 27, 6, 5);
  ...
  // ── Tweed jacket body — dominant silhouette piece. Heather-brown
  // herringbone with darker shoulders. ──
  ...
  // ── Tweed waistcoat visible down the centre — lighter olive
  ...
  // ── Red paisley necktie — aristocratic flash of colour. ──
  ...
  // ── White shirt collar points peeking above the jacket. ──
  ...
  // ── Monocle on right eye — gold circle with chain. ──
  ...
  // ── Walrus moustache — thick, drooping grey. ──
  ...
  // ── DEERSTALKER CAP — the Sherlock-Holmes twin-peaked tweed hat,
  // universally "countryside aristocrat". Two visible peaks (front
  // and back) are the silhouette tell. ──
  g.fillStyle(0x2a1f10, 1);
  ...
  // Back peak (mirror — signature deerstalker tell)
  g.fillStyle(0x2a1f10, 1);
  ...
  // ── Shooting-stick walking cane on the LEFT — signature laird
  // prop. Thick wooden stock with a silver curved handle. ──
  // Stick pole
  ...
  // ── Side-by-side shotgun slung over the RIGHT shoulder. Barrel
  // points up-right, stock visible at the bottom. ──
  // Gun stock — polished walnut
  g.fillStyle(0x3a1a08, 1);
  ...
  // Gun action/receiver — black metal
  g.fillStyle(0x1a1a22, 1);
  g.fillRect(cx + 18, cy - 2, 6, 4);
  // Side-by-side barrels — two parallel pipes going up-right
