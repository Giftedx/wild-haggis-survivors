...
function claimBeforeAnnotation(line, annotation, fact) {
  const claimText = line.slice(0, annotation.index);

  if (fact === 'save.schemaVersion') {
    const versions = [...claimText.matchAll(/\bv(\d+)\b/gi)];
    const version = versions.at(-1);
    return version ? Number(version[1]) : null;
  }

  const candidate = candidateMatches(claimText)
    .find(({ noun }) => FACT_BY_NOUN[noun] === fact);
  return candidate?.claimed ?? null;
}

function findClaimNumber(line, sameLineAnnotation, fact) {
  if (!sameLineAnnotation) return firstNumber(line);

  const afterAnnotation = line.slice(
    sameLineAnnotation.index + sameLineAnnotation[0].length,
  );
  return firstNumber(afterAnnotation)
    ?? claimBeforeAnnotation(line, sameLineAnnotation, fact);
}
...
export async function scanDocs({ root = process.cwd(), facts }) {
  ...
  for (const path of files) {
    ...
    const recordAnnotatedClaim = ({ fact, claimed, line }) => {
      const actual = facts[fact];
      if (claimed !== null && Number.isFinite(actual) && claimed === actual) {
        summary.matched++;
        return;
      }
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      if (precedingAnnotation) {
        if (sameLineAnnotation) {
          recordAnnotatedClaim({ ...precedingAnnotation, claimed: null });
        } else {
          recordAnnotatedClaim({
            fact: precedingAnnotation.fact,
            claimed: findClaimNumber(line, null, precedingAnnotation.fact),
            line: index + 1,
          });
          precedingAnnotation = null;
          continue;
        }
        precedingAnnotation = null;
      }
      ...
      if (sameLineAnnotation) {
        const fact = sameLineAnnotation[1];
        const claimed = findClaimNumber(line, sameLineAnnotation, fact);
        const annotationOnly = line.replace(sameLineAnnotation[0], '').trim() === '';

        if (claimed === null && annotationOnly && index + 1 < lines.length) {
          precedingAnnotation = { fact, line: index + 1 };
        } else {
          recordAnnotatedClaim({ fact, claimed, line: index + 1 });
        }
        continue;
      }
  ...
  for (const finding of findings) {
    if (finding.status === 'MISMATCH') {
      console.log(
        ...
        `claimed ${finding.claimed ?? 'unknown'}, actual ${finding.actual ?? 'unknown'}`,
      );
    } else {
      console.log(
