...
describe('check-doc-facts', () => {
  ...
  it('extracts vNN claims on the annotation line and the following line', async () => {
    const root = await makeFixture({
      'README.md': [
        '<!-- fact:save.schemaVersion --> `whs_save` v23',
        '<!-- fact:save.schemaVersion -->',
        '`whs_save` schema v24',
      ].join('\n'),
    });
  ...
  it('reports annotations that do not resolve to a claim number', async () => {
    const root = await makeFixture({
      'README.md': [
        '| Persistence (3 stores) | details <!-- fact:save.schemaVersion -->',
        '<!-- fact:biomes.count -->',
        '',
      ].join('\n'),
    });
    ...
    expect(result.summary.mismatched).toBe(2);
  });
