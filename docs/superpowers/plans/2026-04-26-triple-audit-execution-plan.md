...
| T306 | **Modal focus abstraction** — consistent keyboard/gamepad for Curse, route cards, NodePrompt, GameOver links. | Shared helper under `src/ui/` or `src/input/` | Tests per modal | ☐ partial — `src/ui/modalFocus.ts` now owns the shared enabled-entry focus rules; `nodePromptNav.ts` delegates to it; `ActIntermissionScene` moved from a `window` digit listener to scene-scoped keyboard with 1/2/3, Arrow/Tab focus, Enter/Space confirm, and gamepad focus/confirm. Remaining surfaces: Curse tile focus and GameOver link/action focus. |
...
| T310 | **Bundle / FMP** — address Vite large-chunk warning: lazy routes for non-critical scenes, dev-only scene not in prod bundle path if possible. | `vite.config.ts`, `main.ts` scene registration | Build output smaller or documented trade | ☑ — `SpriteExportScene` and `CombinationsPreviewScene` removed from static `main.ts` scene imports and lazy-registered through `src/tools/lazyToolScenes.ts` only for `?export`, `?devScenes=1`, or dev hotkeys. Production build now emits separate tool chunks and keeps them out of Workbox precache; main chunk dropped from 1,114.53 kB to ~1,109.56 kB after the T306 follow-up. Affected e2e paths (`sprite-export`, UI audit dev-scene capture) pass. |
...
| **T310 — Bundle / FMP** | Deferred (investigation) | `vite.config.ts` already declares `manualChunks: { 'vendor-phaser': ['phaser', 'eventemitter3'] }` and the SCS i18n overlay is lazy-loaded. The remaining 305 KB-gzip app chunk is dominated by static scene imports in `main.ts` — splitting non-critical scenes (Almanac / Chronicle / Croft / Deeds / Shop / SettingsInput / dev scenes) into side-chunks needs dynamic-import scene registration, a broader refactor that touches every scene boot path. Re-audit confirmed there's no quick `manualChunks` win that defers parse cost without that change. | Dedicated bundle-split PR: convert non-critical scenes to `scene.add(key, () => import(...))` registration so Vite emits them as deferred chunks. Pair with bundle analyser output. |
