...
> | C6 — doc industry / 866 markdown files | partial | `9cbf6ee` master-plan reframe + `docs/superpowers/specs/` trimmed to 3 / `plans/` to 2; `47d1264` archived 5 closed root-level docs (PHASE_0_GATE_NOTES, SCENE_REFACTOR_AUDIT_2026-04-26, SPRITE_AUDIT_2026-04-27, P3_BACKEND_DECISION_MATRIX, BANTER_GAPS), live `docs/` root now 24 files. Research corpus + ADR set + still-active status docs remain by design — broader consolidation past this point is structural, not stale-doc cleanup |
...
> | S7 — `BURNS_EVOLUTION_THRESHOLD` brittle | **closed** | resolved by **intentional hard-pin** at `10` in [`src/core/BalanceConfig.ts`](../src/core/BalanceConfig.ts) (decoupled from `EVOLUTION_RECIPES.length`, re-exported from `save/schema.ts`); decoupling locked by a guard in [`src/data/weapons.test.ts`](../src/data/weapons.test.ts) |
> | S8 — engineering practices honour-system | **closed** | Phase 1 `a189883` (`no-console` + test-focus syntax bans), Phase 2 `9e947d7` (`no-explicit-any: error` in prod / `off` in tests, `no-debugger: error`), Phase 3 today (`eqeqeq` w/ null-check carve-out — original "20+ sweep" miscounted the idiom; working tree was already green) |
...
> **2026-05-10 update — closed, but NOT the way this finding recommended.** The constant is **intentionally hard-pinned at `10`** in [`src/core/BalanceConfig.ts`](../src/core/BalanceConfig.ts) and re-exported from [`src/utils/save/schema.ts`](../src/utils/save/schema.ts) for back-compat — it is **decoupled** from `EVOLUTION_RECIPES.length`, not derived from it. The rationale block on the constant explains why: Burns's gate was authored around "all weapon-family evolutions in a run", so a rhythm-coupled 11th recipe (the Pibroch Hammer) would silently tighten the unlock from 10→11 and break the achievement contract for players mid-progress. A vitest guard in [`src/data/weapons.test.ts`](../src/data/weapons.test.ts) locks the decoupling (`BURNS_EVOLUTION_THRESHOLD === 10` and `EVOLUTION_RECIPES.length >= BURNS_EVOLUTION_THRESHOLD`) so a future recipe can't silently re-bind the gate. Achievement copy interpolates `{count}` from the constant (see `descriptionVars` on `ach_burns_beastie_unlock` + `{count}` placeholders in `i18n/achievement.ts` + `i18n.scs/achievement.ts`); lifting the gate is now a deliberate two-step (bump the constant *and* the copy). The original "make it derived" recommendation below is superseded by this decision.
