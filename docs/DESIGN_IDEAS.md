...
- ~~**Sporran Deck**~~ — ✅ Phase 0 + Phase 1 shipped 2026-05-09 (Phase 0 helper `eabe2a6` `src/systems/sporranDeck.ts` + `sporranDeck.test.ts` + 11-card pool `src/data/sporranCards.ts`; Phase 1 UI `6275720` `src/scenes/SporranScene.ts` + `sporranTileLayout.ts` + `game/sporranRunStart.ts` + EN + SCS i18n). Pre-run alternative to the Curse picker — opt-in via the "DRAW THE SPORRAN" link on `CurseScene`. Player drafts 7 cards from `ALL_SPORRAN_CARDS` (5 curses wrapping CURSES, 4 boons, 2 quirks) and keeps 3; the picked IDs ride the GameScene init payload as `pickedSporranIds`. `buildSporranRunStartPlan` mutates `RunModifiers` between curse + seasonal application; `applySporranRunStartPostSpawn` heals after Player construction. Resumed runs short-circuit so re-applies don't compound. Replay-deterministic via *recorded picks*, not the cosmetic Date.now() shuffle (T1 contract). Phase 2 follow-ups: DOM-focus mirror parity (a11y), `RunHistoryEntry.sporranPicks` persistence + schema bump v18→v19, pool expansion (rare/seasonal/variant-keyed cards + the deferred `quirk_haggis_blooded` once `RunModifiers.damageMult` exists), URL `?sporran=1` auto-route flag, balance/telemetry pass.
