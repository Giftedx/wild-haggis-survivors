...
| **W71** Skeletal animation rig | 🟡 Phase 0 prototype shipped 2026-04-22; Phase 1 enemy animation + Phase 2 secondary motion shipped 2026-04-23. **2026-05-28: all gameplay enemies animated** — 19 boss frame drawers + beithir + 5 minion/swarm drawers (midge, midgie_swarm, nest, tourist_ghost, spectre_legionary). Remaining without drawers: `auld_reekie` (architectural blocker), `deep_fryer` (structural sprite only — static hazard, no walk states). `berserker` ✅ frame drawer landed 2026-05-28 (heavier offsets on `drawAngryScotsmanBody`). |
