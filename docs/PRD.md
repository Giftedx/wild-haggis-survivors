...
| **W71** Skeletal animation rig | 🟡 Phase 0 prototype shipped 2026-04-22; Phase 1 enemy animation + Phase 2 secondary motion shipped 2026-04-23. **2026-05-28: all gameplay enemies animated** — 19 boss frame drawers + beithir + 5 minion/swarm drawers (midge, midgie_swarm, nest, tourist_ghost, spectre_legionary). All gameplay enemies now animated. `berserker` ✅ `deep_fryer` ✅ `auld_reekie` ✅ `traffic_cone_totem` ✅ (all landed 2026-05-28; `auld_reekie` required square-canvas refactor from 48×56 → 60×60; `traffic_cone_totem` was the last unanimated data key — speed-0 rooted enemy, idle/hurt/dying authored, no walking). |
...
- **Codebase restructure (2026-04-30)** — Phases 0–7 SHIPPED by 2026-05-09; T401 GameScene decomposition is the historical chain that fed it (3526 → 1819 LOC at the 2026-05-10 baseline). **GameScene facade decomposition SHIPPED 2026-06-28** — `create()` cut from a ~930-line monolith to a ~57-line four-phase sequence (`installWorldAndAtmosphere` → `installPlayerAndRunStart` → `installCombatAndUpgrades` → `installUiLandmarksAndFlow`) + the large inline install hook bags moved to sibling `build*Hooks.ts` / `build*Deps.ts` builders; file went 2186 → 1192 LOC (−45%), behaviour preserved (full vitest + e2e green, RNG order preserved for replay determinism). The ≤1200 facade target is now MET; the hard ceiling was ratcheted 2200 → 1400 to lock it (see [`docs/LOC_BUDGET.md`](LOC_BUDGET.md)).
