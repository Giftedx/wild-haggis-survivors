...
| **T1** | **Deterministic replay** | S | **Record + best-effort playback shipped 2026-04-17 / 18** (ADR-0002 + addendum). Phase 1: `ReplayRecorder` + `ReplayBlob` + save v5 + GameScene record wire behind `whs_replay_mode` flag; two non-determinism blockers fixed (HazardZones RDG, SpawnSystem.pauseSpawnsFor). Phase 2: `IInput` interface extraction, `ReplayInput` playback driver injected into Player via DI, Chronicle ▶ Watch button per entry with a valid `replay` blob, blob exhaustion auto-returns to Chronicle. v1 playback is best-effort — drift expected (variable Phaser delta, curse/route/meta-stats not yet bundled). Phase 3 (fixed-step + v2 blob schema) tracked in ADR-0002 addendum. | Not a crash-proof system. Not a cross-version bridge — replays valid within one build. No audio re-record — playback re-schedules from the same inputs. | If desync rate >1% across a playtest suite once fixed-step playback lands, shelve until root cause understood. |
