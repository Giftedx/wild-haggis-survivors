...
**Trigger wiring — 3 of 9 sub-pools live, 6 deferred.**
...
- **Live** (`GameScene.trySpawnAncestralEcho.onTouch`): `lineage_moment` — "John Anderson My Jo" fires the tick after the echo pity-reward toast. Naturally once-per-run: `consumeLastDeath` + the `ancestralEcho` guard gate the spawn to one per run, and echo self-despawns on touch. No extra throttle required.
...
- **`burns_citation` wiring** — 3 of 9 sub-pools live (loch_moment + highland_moment on biome moor_moments; lineage_moment on ancestral-echo touch); 6 deferred per the starvation analysis above. Remaining wires are priority-starved at their natural trigger surfaces — each needs a distinct post-resolve tick or per-event reservation work.
