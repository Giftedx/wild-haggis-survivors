 ...
 *
 * The Stoor Worm is an Orcadian sea-serpent; `manualSpawn: true` so it never
 * enters the regular wave pool. This test uses `spawnBossManually` to bypass
 * time-based routing and exercises the full kill pipeline.
 *
 * Verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline (phase-1 DR
 *      is 80% but 999 999 damage still clears 8 000 HP).
 *   3. GameScene.bossKilledKeys records 'stoor_worm' after the kill.
 *
...
test.describe('Stoor Worm boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('stoor_worm', px + 80, py);
      return true;
    });
    expect(spawned, 'spawnBossManually must succeed').toBe(true);

    // Wait for entity to materialise (SpawnSystem emits a 1500ms warning
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'stoor_worm') ?? false;
    }, undefined, { timeout: 8_000 });

    await healPlayer();

    // Phase-1 DR is 80% — 999 999 damage still clears 8 000 HP.
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'stoor_worm');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'stoor_worm must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('stoor_worm');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include stoor_worm').toBe(true);
