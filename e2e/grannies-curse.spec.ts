 ...
 *
 * Unit tests cover the spread/pierce math + hex-bolt projectile lifecycle,
 * but this smoke checks the Phaser-bound glue: once `grannies_curse` is
 * equipped in a live run with real enemies present, does it ever fire
 * (observable as its cooldown being stamped from 0 → >0)?
 *
 * Sister of `e2e/shinty-stick.spec.ts` / `e2e/sgian-dubh.spec.ts` — same
 * shape; only the weapon key + cooldown differ. Granny's Curse base
 * cooldown is 1600ms (see `src/data/weapons.ts`), so 3s of polling
 * covers the first fire comfortably.
 *
...
test.describe("granny's curse (Highland Horrors B6)", () => {
  ...
  test('equipped grannies_curse stamps cooldown when enemies are present', async ({ page }) => {
    ...
    const added = await page.evaluate(() => {
      ...
      scene.weaponSystem.addWeapon('grannies_curse');
      const ws = scene.weaponSystem.getWeapons() as Array<{ config?: { key?: string } }>;
      return ws.some((w) => w.config?.key === 'grannies_curse');
    });
    expect(added, 'grannies_curse must be in active weapons').toBe(true);
    ...
    // Poll for cooldown stamp — when grannies_curse fires, cooldownRemaining
    // flips to >0. Base cooldown is 1600ms; 3s window covers it.
    const stamped = await page.evaluate(async () => {
      const readCooldown = (): number => {
        ...
        const w = weapons.find((x) => x.config?.key === 'grannies_curse');
    ...
    expect(stamped, 'grannies_curse cooldown must stamp within 3s once enemies exist').toBeGreaterThan(0);
