 ...
 *
 * The Ninth Legion is a post-bell-only wave-boss (`postBellOnly: true`,
 * `spawnTimeSec: 9999`). This test uses `spawnBossManually` to bypass the
 * post-bell gate and exercises the full kill pipeline including the shroud
 * DR mechanic (90% reduction until phase 3 — 999 999 damage still clears
 * 5 000 HP).
 *
 * Verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline.
 *   3. GameScene.bossKilledKeys records 'ninth_legion' after the kill.
 *
...
test.describe('Lost Ninth Legion boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    // Bypass the post-bell gate by spawning manually — same path used
    // by CailleachGauntletScheduler for other post-bell exclusives.
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('ninth_legion', px + 80, py);
      return true;
    });
    expect(spawned, 'spawnBossManually must succeed').toBe(true);

    // Wait for entity to materialise (1500ms warning beat before active).
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'ninth_legion') ?? false;
    }, undefined, { timeout: 8_000 });

    await healPlayer();

    // Shroud DR is 90% — 999 999 damage still clears 5 000 HP.
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'ninth_legion');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'ninth_legion must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('ninth_legion');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include ninth_legion').toBe(true);
