 ...
 *
 * A post-bell Tier-3 boss (postBellOnly: true) with a 'storm_phases'
 * behaviour: haar veil → ice fury → hail storm across three escalating
 * phases. HP 5800, speed 55, scale 2.8×.
 *
 * This smoke verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline.
 *   3. GameScene.bossKilledKeys records 'storm_cailleach' after the kill.
 *
...
test.describe('Cailleach of the Storm boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('storm_cailleach', px, py);
      return true;
    });
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'storm_cailleach') ?? false;
    ...
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'storm_cailleach');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'storm_cailleach must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('storm_cailleach');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include storm_cailleach').toBe(true);
