...
/**
 * Auld Reekie Ghaist boss — spawn + kill smoke.
 *
 * An urban-family boss at 18:30 with 'auld_reekie' behaviour: slow glide +
 * lantern orb/fan + lamp-post blink teleport + gas pulse slow. This test
 * bypasses the 18:30 time-based spawn and calls `spawnBossManually` directly
 * — the same public API used by CailleachGauntletScheduler.
 *
 * Verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline.
 *   3. GameScene.bossKilledKeys records 'auld_reekie' after the kill.
 *
 * State-machine behaviour is covered by auldReekieBehaviour.test.ts.
 * Chromium-only — FF/WK headless WebGL flakes.
 */

const META_SAVE_VERSION = 11;

test.describe('Auld Reekie Ghaist boss — spawn and kill', () => {
  test.skip(
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('auld_reekie', px + 80, py);
      return true;
    });
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'auld_reekie') ?? false;
    ...
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'auld_reekie');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'auld_reekie must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('auld_reekie');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include auld_reekie').toBe(true);
