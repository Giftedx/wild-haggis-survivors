 ...
 *
 * The Laird is a standard chase boss at 15:00 (spawnTimeSec 900).
 * This test uses `spawnBossManually` to bypass time routing and
 * exercises the full kill pipeline.
 *
 * Verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline (HP 3500,
 *      no DR — 999 999 damage clears it in one hit).
 *   3. GameScene.bossKilledKeys records 'the_laird' after the kill.
 *
...
test.describe('The Laird boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('the_laird', px + 80, py);
      return true;
    });
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'the_laird') ?? false;
    }, undefined, { timeout: 8_000 });

    await healPlayer();

    // HP 3500, no DR — 999 999 damage clears it in one hit.
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'the_laird');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'the_laird must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('the_laird');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include the_laird').toBe(true);
