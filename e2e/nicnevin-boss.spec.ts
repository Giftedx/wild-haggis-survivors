 ...
 *
 * Nicnevin is a `spawner` behaviour boss at 12:30 (spawnTimeSec 750).
 * She summons unseelie_fiddler minions and triggers a Wild Hunt gem-pull
 * at 50% HP. This test uses `spawnBossManually` to bypass time routing
 * and exercises the full kill pipeline.
 *
 * Verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline (HP 3200,
 *      no DR — 999 999 damage clears it in one hit).
 *   3. GameScene.bossKilledKeys records 'nicnevin' after the kill.
 *
...
test.describe('Nicnevin boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('nicnevin', px + 80, py);
      return true;
    });
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'nicnevin') ?? false;
    }, undefined, { timeout: 8_000 });

    await healPlayer();

    // HP 3200, no DR — 999 999 damage clears it in one hit even if the
    // Wild Hunt 50%-HP gem-pull phase triggers mid-pipeline.
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'nicnevin');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'nicnevin must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('nicnevin');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include nicnevin').toBe(true);
