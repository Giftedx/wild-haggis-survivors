 ...
 *
 * Bealltainn's Tribute: a burning wicker effigy with a haggis inside.
 * Post-bell exclusive (postBellOnly: true). Phase 1: outward fire-ring.
 * Phase 2: ember scatter + speed boost. Transition ignition burst on HP
 * crossing 55%. HP 3600, speed 60, scale 2.3×.
 *
 * This smoke verifies:
 *   1. Boss appears in the enemy group after a manual spawn call.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline.
 *   3. GameScene.bossKilledKeys records 'wicker_haggis' after the kill.
 *
...
test.describe('Wicker Haggis boss — spawn and kill', () => {
  ...
  test('spawns via spawnBossManually and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    const spawned = await page.evaluate(() => {
      ...
      gs.getSpawnSystem().spawnBossManually('wicker_haggis', px, py);
      return true;
    });
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'wicker_haggis') ?? false;
    ...
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'wicker_haggis');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'wicker_haggis must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('wicker_haggis');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include wicker_haggis').toBe(true);
