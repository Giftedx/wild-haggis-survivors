 ...
 * wiring smoke.
 *
 * Unit tests cover the bouncing math + `fireBouncing` dispatch fork
 * (haggis_ball vs shinty_ball texture). This smoke covers the
 * Phaser-bound glue: once `shinty_stick` is equipped in a live run
 * with enemies present, does it ever fire (observable as its
 * cooldown being stamped from 0 → >0)?
 *
 * Sister of `e2e/sgian-dubh.spec.ts` — same shape; only the weapon
 * key + behavior path differ.
 *
...
test.describe('shinty stick (DESIGN_IDEAS §1+§5)', () => {
  ...
  test('equipped shinty_stick stamps cooldown when enemies are present', async ({ page }) => {
    ...
    const added = await page.evaluate(() => {
      ...
      scene.weaponSystem.addWeapon('shinty_stick');
      const ws = scene.weaponSystem.getWeapons() as Array<{ config?: { key?: string } }>;
      return ws.some((w) => w.config?.key === 'shinty_stick');
    });
    expect(added, 'shinty_stick must be in active weapons').toBe(true);
    ...
    // Poll for cooldown stamp — when shinty_stick fires, cooldownRemaining
    // flips to >0. Shinty Stick base cooldown is 1700ms (see
    // src/data/weapons.ts), so 3s of polling comfortably covers the
    // first fire even at heavy frame jitter.
    const stamped = await page.evaluate(async () => {
      const readCooldown = (): number => {
        ...
        const w = weapons.find((x) => x.config?.key === 'shinty_stick');
    ...
    expect(stamped, 'shinty_stick cooldown must stamp within 3s once enemies exist').toBeGreaterThan(0);
