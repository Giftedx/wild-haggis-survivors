...
/**
 * Wallace Sword (Highland Horrors B6) heavy-arc-sweep weapon wiring smoke.
 *
 * Unit tests cover the wide-arc damage + heavy-knockback math, but this
 * smoke checks the Phaser-bound glue: once `wallace_sword` is equipped in
 * a live run with real enemies present, does it ever fire (observable as
 * its cooldown being stamped from 0 → >0)?
 *
 * Sister of `e2e/shinty-stick.spec.ts` / `e2e/sgian-dubh.spec.ts` — same
 * shape; only the weapon key + cooldown differ. Wallace Sword base
 * cooldown is 3400ms (see `src/data/weapons.ts`), so the poll window
 * is widened to 5s to comfortably cover a single fire even at heavy
 * frame jitter.
 *
...
test.describe('wallace sword (Highland Horrors B6)', () => {
  test.skip(
    ({ browserName }) => browserName !== 'chromium',
    'cross-browser headless WebGL flakes; chromium covers the smoke',
  );

  test('equipped wallace_sword stamps cooldown when enemies are present', async ({ page }) => {
    ...
    const added = await page.evaluate(() => {
      ...
      scene.weaponSystem.addWeapon('wallace_sword');
      const ws = scene.weaponSystem.getWeapons() as Array<{ config?: { key?: string } }>;
      return ws.some((w) => w.config?.key === 'wallace_sword');
    });
    expect(added, 'wallace_sword must be in active weapons').toBe(true);
    ...
    // Poll for cooldown stamp — when wallace_sword fires, cooldownRemaining
    // flips to >0. Base cooldown is 3400ms; 5s window covers it with
    // headroom for frame jitter.
    const stamped = await page.evaluate(async () => {
      const readCooldown = (): number => {
        ...
        const w = weapons.find((x) => x.config?.key === 'wallace_sword');
    ...
    expect(stamped, 'wallace_sword cooldown must stamp within 5s once enemies exist').toBeGreaterThan(0);
