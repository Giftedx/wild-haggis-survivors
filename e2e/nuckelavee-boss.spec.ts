...
/**
 * Nuckelavee boss — spawn + kill smoke.
 *
 * The Nuckelavee is an Orcadian sea-demon boss that spawns at 17:00
 * (spawnTimeSec: 1020) and retreats from fresh-water heal patches
 * (tickNuckelaveeRetreat in tickFrameWorld.ts).
 *
 * This smoke verifies:
 *   1. Boss appears in the enemy group at the expected game time.
 *   2. `takeDamageWithKillEvents` drives the full kill pipeline.
 *   3. GameScene.bossKilledKeys records 'nuckelavee' after the kill.
 *
 * The retreat mechanic is a per-frame velocity override; unit coverage
 * lives in nuckelaveeRetreat.test.ts. The e2e only checks spawn + kill
 * wiring so CI stays fast.
 *
...
test.describe('Nuckelavee boss — spawn and kill', () => {
  test.skip(
    ({ browserName }) => browserName !== 'chromium',
    'cross-browser headless WebGL flakes; chromium covers the smoke',
  );

  test('spawns at 17:00 and registers in bossKilledKeys on death', async ({ page }) => {
    ...
    // Advance to 17:00 — Nuckelavee spawns at spawnTimeSec 1020 (= 17 min).
    await healPlayer();
    await page.evaluate(() => {
    ...
    // Wait for Nuckelavee to materialise in the enemy group.
    ...
    await page.waitForFunction(() => {
      ...
      return gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .some((e) => e.active && e.getEnemyKey?.() === 'nuckelavee') ?? false;
    ...
    const bossKilled = await page.evaluate(() => {
      ...
      const boss = gs?.spawnSystem?.getEnemyGroup?.().getChildren()
        .find((e) => e.active && e.getEnemyKey?.() === 'nuckelavee');
      if (!boss) return false;
      boss.takeDamageWithKillEvents?.(999_999);
      return true;
    });
    expect(bossKilled, 'nuckelavee must be found and killed').toBe(true);
    ...
    const killRegistered = await page.waitForFunction(() => {
      ...
      return (gs?.bossKilledKeys ?? []).includes('nuckelavee');
    }, undefined, { timeout: 5_000 });

    expect(await killRegistered.jsonValue(), 'bossKilledKeys must include nuckelavee').toBe(true);
